...
const BASE_API_URL = 'http://localhost:3000/api/news';
...
function timeAgo(dateString) {
    ...
    const seconds = Math.floor((now - date) / 3000);
...
function createNewsElement(newsItem, type) {
    ...
    const getImageUrl = (url) => {
        return url.startsWith('/uploads/') ? `http://localhost:3000/api/news${url}` : url;
    };
...
newsForm.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (editingId) {
            // UPDATE News Item
            response = await fetch(`${BASE_API_URL}/${editingId}`, {
                method: 'PUT',
...
async function fetchNewsDetailAndComments(newsId) {
    ...
    try {
        const newsResponse = await fetch(`${BASE_API_URL}/${newsId}`);
        if (!newsResponse.ok) {
        ...
        const commentsResponse = await fetch(`${BASE_API_URL}/${newsId}/comments`);
...
commentForm.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (editCommentId) {
            // UPDATE Comment
            response = await fetch(`${BASE_API_URL}/${newsId}/comments/${editCommentId}`, {
                method: 'PUT',
            ...
            // CREATE Comment
            response = await fetch(`${BASE_API_URL}/${newsId}/comments`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(commentData)
            });
...
document.addEventListener('click', async function(e) {
    ...
    if (target.classList.contains('fa-trash') && target.dataset.action === 'delete') {
        ...
        if (newsItemAuthorId === MY_POSTS_AUTHOR_ID) {
            if (confirm('Are you sure you want to delete this news item?')) {
                ...
                try {
                    const response = await fetch(`${BASE_API_URL}/${newsIdToDelete}`, {
                        method: 'DELETE'
                    });
        ...
        if (newsItemAuthorId === MY_POSTS_AUTHOR_ID) {
            ...
            try {
                const response = await fetch(`${BASE_API_URL}/${newsIdToEdit}`);
                if (!response.ok) {
                ...
                if (newsItem.imageUrl) {
                    ...
                    currentImagePreview.src = newsItem.imageUrl.startsWith('/uploads/') ? `http://localhost:3000/api/news${newsItem.imageUrl}` : newsItem.imageUrl;
                    imagePreviewContainer.style.display = 'flex';
                } else {
                    imagePreviewContainer.style.display = 'none';
                    currentImagePreview.src = '';
    ...
    // --- Comment Actions (Edit/Delete) ---
        ...
        try {
            const commentResponse = await fetch(`${BASE_API_URL}/${newsId}/comments`);
            ...
            if (commentToDelete && commentToDelete.authorId === MY_POSTS_AUTHOR_ID) {
                ...
                if (confirm('Are you sure you want to delete this comment?')) {
                    ...
                    const response = await fetch(`${BASE_API_URL}/${newsId}/comments/${commentIdToDelete}`, {
                        method: 'DELETE'
                    });
